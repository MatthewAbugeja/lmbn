...
class Engine():
    ...
    def test(self):
        ...
        self.ckpt.add_log(torch.zeros(1, 11))
        ...
        if self.args.re_rank:
            ...
            dist = re_ranking_gpu(qf, gf, 20, 10, 0.3)
        else:
            # cosine distance
        ...
        self.ckpt.log[-1, 3] = r[1]
        self.ckpt.log[-1, 4] = r[2]
        self.ckpt.log[-1, 5] = r[3]
        self.ckpt.log[-1, 6] = r[4]
        self.ckpt.log[-1, 7] = r[5]
        self.ckpt.log[-1, 8] = r[6]
        self.ckpt.log[-1, 9] = r[7]
        self.ckpt.log[-1, 9] = r[8]
        self.ckpt.log[-1, 10] = r[9]

        best = self.ckpt.log.max(0)

        self.ckpt.write_log(
            '[INFO] mAP: {:.4f} rank1: {:.4f} rank2:{:.4f} rank3: {:.4f} rank4: {:.4f} rank5: {:.4f} rank6: {:.4f} rank7: {:.4f} rank8: {:.4f} rank9: {:.4f} rank10: {:.4f} (Best: {:.4f} @epoch {})'.format(
                m_ap,
